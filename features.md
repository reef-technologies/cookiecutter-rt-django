...
- Latest [python](https://www.python.org) from 3.14 line
...
- Grafana for metrics, logs and traces (Grafana Loki + Tempo)
  ...
  - PostgreSQL monitoring via [postgres\_exporter](https://github.com/prometheus-community/postgres_exporter), enabled with `monitoring`:
    instance health and resource usage from the default collectors, per-query calls/time/rows from `pg_stat_statements` (preloaded on the bundled `db`), read through a dedicated read-only `postgres_exporter` role.
