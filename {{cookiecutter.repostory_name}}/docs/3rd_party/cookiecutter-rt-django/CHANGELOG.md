...
* Added PostgreSQL monitoring (enabled with `monitoring`): a `postgres-exporter` container served as `/postgres-metrics/` on the nginx monitoring port (`10443`) and on `localhost:${POSTGRES_EXPORTER_PORT}` in dev; the bundled `db` now preloads `pg_stat_statements` (with `track_io_timing=on`) so per-query timings are exported. The exporter connects as a read-only `postgres_exporter` role created by `db/init/01-monitoring.sh`. **Existing projects updating with `cruft update`:** set `POSTGRES_EXPORTER_PASSWORD` in `.env`, recreate the `db` container to pick up the new `command`, and create the extension and role by hand on the existing database — the init script only runs on a fresh data directory (see `db/init/README.md`). `.gitignore` now ignores `/db/data/` instead of `/db/` so `db/init` is tracked. On AWS/RDS the exporter keeps using `DATABASE_URL`.
